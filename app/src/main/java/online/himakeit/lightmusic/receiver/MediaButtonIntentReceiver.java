package online.himakeit.lightmusic.receiver;
...
import online.himakeit.lightmusic.ui.activity.MainActivity;
...
public class MediaButtonIntentReceiver extends WakefulBroadcastReceiver {
        ...
        public void handleMessage(final Message msg) {
            switch (msg.what) {
                case MSG_LONGPRESS_TIMEOUT:
                    LogUtils.show(TAG, "Handling longpress timeout, launched " + mLaunched);
                    if (!mLaunched) {
    ...
    private static void acquireWakeLockAndSendMessage(Context context, Message msg, long delay) {
        ...
        LogUtils.show(TAG, "Acquiring wake lock and sending " + msg.what);
    ...
    private static void releaseWakeLockIfHandlerIdle() {
        if (mHandler.hasMessages(MSG_LONGPRESS_TIMEOUT)
            ...
            LogUtils.show(TAG, "Handler still has messages pending, not releasing wake lock");
            return;
        }

        if (mWakeLock != null) {
            LogUtils.show(TAG, "Releasing wake lock");
            mWakeLock.release();
            mWakeLock = null;
        }
    }
                ...
                if (action == KeyEvent.ACTION_DOWN) {
                    if (mDown) {
                        ...
                        if (keycode == KeyEvent.KEYCODE_HEADSETHOOK) {
                            ...
                            mClickCounter++;
                            LogUtils.show(TAG, "Got headset click, count = " + mClickCounter);
